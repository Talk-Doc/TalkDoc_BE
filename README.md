...
| `DURATION` | 언제부터 아팠어요? | `CARD_SELECT` | 오늘부터 / 어제부터 / 2~3일 전부터 / 1주일 전부터 / 2주 이상 / 한 달 이상 |
| `SEVERITY` | 얼마나 아파요? | `CARD_SELECT` | 거의 없음 / 약간 / 보통 / 심함 / 참기 힘듦 |
| `FREQUENCY` | 얼마나 자주 그래요? | `CARD_SELECT` | 계속 / 하루 여러 번 / 하루 한 번 / 이틀에 한 번 / 가끔 |
| `YES_NO` | 약 드세요? 임신 중이세요? | `CARD_SELECT` | 네 / 아니요 / 잘 모르겠어요 |
| `CHOICE` | 어느 쪽 다리가 아파요? 통증이 어떤 느낌이에요? | `CARD_SELECT` | **LLM이 질문마다 생성** (2~6개, 20자 이내). 예: 왼쪽 / 오른쪽 / 양쪽 |

`CHOICE`는 고정표에 없는 객관식 질문을 위한 유형입니다. 의도 분류와 같은 Gemini 호출에서 선택지를
함께 받아(`card_options`) 서버가 정리(공백·번호 제거, 중복 제거, 최대 6개)한 뒤 내려줍니다. LLM이
선택지를 2개 미만으로 주면 `OTHER`(지원하지 않는 질문)로 처리합니다. 고정표 유형은 LLM이 선택지를
보내도 무시하고 서버 표를 씁니다.
