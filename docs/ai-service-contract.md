...
`Intent.java`에 정의된 수어 어휘는 TalkDoc-VisionAI 모델이 실제로 학습한 15개 단어와 일치한다.
원래 목표였던 20개 중 가슴, 허리, 기침, 구토, 알레르기는 모델에 없어 2026-09-14 팀 결정으로
어휘에서 제외했다(모델이 추가 학습되면 다시 넣는다).

| 의도 (Intent) | 라벨 |
|---------------|------|
| `BODY_LOCATION` (신체 부위, 5개) | 머리, 목, 배, 팔, 다리 |
| `SYMPTOM` (증상, 6개) | 아프다, 어지럽다, 설사, 숨차다, 답답하다, 붓다 |
| `HISTORY_STATE` (병력/상태, 4개) | 약, 감기, 임신, 당뇨병 |

모델 `classes.json` 순서: 감기, 다리, 답답하다, 당뇨병, 머리, 목, 배, 붓다, 설사, 숨차다,
아프다, 약, 어지럽다, 임신, 팔.

카드 선택 의도(`DURATION`, `SEVERITY`, `FREQUENCY`, `YES_NO`)와 `OTHER`는 수어 어휘가 없다.
카드 의도는 수어 인식 없이 고정 선택지로 답하며(README "답변 유형" 참고), `OTHER`는 지원 대상 외
질문이다. 어느 경우에도 `TalkDoc-VisionAI`는
